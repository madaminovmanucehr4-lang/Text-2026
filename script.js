...
function initFakeLeaderboard() {
    if (!localStorage.getItem('fake_leaderboard')) {
        let fakeData = [{ name: "KING", score: 40000 }, { name: "Shadow_Pro", score: 30000 }, { name: "Legendary", score: 20000 }, { name: "MegaMind", score: 10000 }];
...
window.showLeaderboard = function() {
    ...
    board = board.map(player => {
        player.score += Math.floor(Math.random() * 50) + 5;
        return player;
    });
...
function updateUI() {
    ...
    // --- ЛОГИКА ДЛЯ ТЕКСТА В СЕРДЦЕ ---
    scoreHeartEl.innerText = heartScore;
    
    // Считаем сколько цифр в числе
    const len = heartScore.toString().length;

    if (len <= 4) {
        scoreHeartEl.style.fontSize = "14px"; // Обычный размер
    } else if (len === 5) {
        scoreHeartEl.style.fontSize = "18px"; // Для чисел типа 10 000
    } else if (len === 6) {
        scoreHeartEl.style.fontSize = "20px"; // Для чисел типа 100 000
    } else if (len >= 7) {
        scoreHeartEl.style.fontSize = "20px"; // Для 1 000 000 и выше
    }
    // ----------------------------------
