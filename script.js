...
function saveGameState() {
    const gameState = {
        ...
        currentShapesData: currentShapesData // Сохраняем даже текущие фигуры внизу
    };
...
function loadGameState() {
    ...
    if (saved) {
        ...
        lvlProgress = data.lvlProgress;
        currentShapesData = data.currentShapesData;
        
        renderGridFromMemory();
        renderDock();
        updateUI();
        return true;
    }
...
function spawnNewSet() {
    ...
    renderDock();
    saveGameState(); // Сохраняем новые фигуры
}
...
function renderDock() {
    ...
    currentShapesData.forEach((shape, index) => {
        ...
        container.id = `shape-container-${index}`; // Добавили ID
...
function startDrag(e, shape, idx) {
    ...
    // Скрываем оригинал внизу (делаем прозрачным), пока тащим
    ...
    const end = (me) => {
        ...
        // ЕСЛИ НЕ ПОСТАВИЛИ — возвращаем яркость фигуре внизу
        if (!placed) {
...
function finalizeTurn() {
    ...
    else { renderDock(); saveGameState(); } // Сохраняем после хода
...
function checkLines() {
    ...
    if (rs.length > 0 || cs.length > 0) {
        rs.forEach(r => {
            for(let c=0; c<8; c++) { animateCell(r, c); grid[r][c] = null; }
            heartScore += 100;
        });
        cs.forEach(c => {
            for(let r=0; r<8; r++) { if (grid[r][c] !== null) { animateCell(r, c); grid[r][c] = null; } }
            heartScore += 50;
        });
...
function updateUI() {
    ...
    saveGameState(); // Авто-сохранение при любом обновлении
}
...
function checkGameOver() {
    ...
    if(!hasMove && heartScore < 150 && currentShapesData.some(s => s !== null)) {
        isLocked = true;
        setTimeout(() => gameOverScreen.classList.remove('hidden'), 1500);
    }
}

window.buyPower = function() {
    // 1. Проверяем, хватает ли очков
    if (heartScore < 150) return;

    // 2. Ищем первый свободный слот ИЛИ слот, где НЕ бомба
    // (Чтобы можно было купить до 3-х бомб в разные слоты)
    let targetIndex = -1;
    for (let i = 0; i < currentShapesData.length; i++) {
        // Если в слоте пусто или там обычная фигура — мы можем поставить туда бомбу
        if (currentShapesData[i] === null || !currentShapesData[i].isBomb) {
            targetIndex = i;
            break; 
        }
    }

    // 3. Если все слоты уже заняты бомбами, ничего не делаем
    if (targetIndex === -1) {
        alert("Все слоты заняты бомбами!"); // Можно убрать alert, если мешает
        return; 
    }

    // 4. Только теперь списываем очки и ставим бомбу
    heartScore -= 150;
    
    currentShapesData[targetIndex] = { 
        n: 'bomb', 
        m: [[1]], 
        isBomb: true, 
        id: Math.random() 
    ...
    // 5. Обновляем интерфейс и сохраняем
    renderDock();
    updateUI();
    saveGameState();
};
// Вставь это в начало своего JS файла, где объявляешь переменные
...
function finalizeTurn() {
    checkLines();
    if (currentShapesData.every(s => s === null)) { 
        lvlProgress += 10; 
        spawnNewSet(); 
    } else { 
        renderDock(); 
    ...
    if (lvlProgress >= 100) { 
        ...
        setTimeout(() => {
            ...
            updateUI(); 
            spawnNewSet();
        }, 2000); // Теперь экран висит 2 секунды, чтобы игрок кайфанул
    }
    updateUI();
}
...
function updateUI() {
    ...
    // --- УМНЫЙ ШРИФТ ДЛЯ СЕРДЦА ---
    const scoreStr = heartScore.toString();
    if (scoreStr.length <= 4) {
        scoreHeartEl.style.fontSize = "18px"; // Маленькое число — крупный шрифт
    } else if (scoreStr.length <= 6) {
        scoreHeartEl.style.fontSize = "14px"; // Тысячи — чуть меньше
    } else if (scoreStr.length <= 8) {
        scoreHeartEl.style.fontSize = "11px"; // Миллионы — еще меньше
    } else {
        scoreHeartEl.style.fontSize = "9px";  // Очень много — самый мелкий
    }
    // ------------------------------
...
function bumpHeart() {
    const heart = document.querySelector('.neon-heart-wrapper');
    if (heart) {
        heart.classList.remove('heart-bump');
        void heart.offsetWidth; // Магия, чтобы анимация сработала еще раз
        heart.classList.add('heart-bump');
    }
}
